...
class Laba6:
    ...
    def main(self):
        ...
        # Кохрен
        self.startcochrane = timeit.default_timer()
        self.F1 = self.M - 1
        self.F2 = self.N
        self.q = 0.05
        self.cochrane()
        self.endcochrane = timeit.default_timer()
        self.timecochrane = self.endcochrane - self.startcochrane
        if (self.GP < self.GT):
        ...
        # Стьюдент
        self.startstudent = timeit.default_timer()
        self.student()
        ...
        self.endstudent = timeit.default_timer()
        self.timestudent = self.endstudent - self.startstudent
        # Фішер
        self.startfisher = timeit.default_timer()
        self.fisher()
        self.endfisher = timeit.default_timer()
        self.timefisher = self.endfisher - self.startfisher
        if (self.FT > self.FP):
            print("FT = {0:.2f} > FP = {1:.2f} - рівняння регресії адекватно оригіналу".format(self.FT, self.FP))
            print("\nЧас виконання кожної статистичної перевірки: \nПеревірка за критерієм Кохрена = {0} сек; \nПеревірка за критерієм Стьюдента = {1} сек; \nПеревірка за критерієм Фішера = {2} сек;".format(round(self.timecochrane,6),round(self.timestudent,6),round(self.timefisher,6)))
            return True
        if (self.FP > self.FT):
